...
3. El domingo NO debe tener ejercicios.`;
  ...
  // Ajuste: reglas de número de ejercicios según la duración de la sesión
  let reglasEjerciciosDuracion = '';
  if (entrenamiento.duracionSesion && typeof entrenamiento.duracionSesion === 'number') {
    const minEjercicios = Math.max(3, Math.floor(entrenamiento.duracionSesion / 12));
    const maxEjercicios = Math.min(10, Math.ceil(entrenamiento.duracionSesion / 7));
    reglasEjerciciosDuracion = `IMPORTANTE: Para cada día normal, el número de ejercicios debe estar entre ${minEjercicios} y ${maxEjercicios}, ajustando el volumen total para que la sesión dure aproximadamente ${entrenamiento.duracionSesion} minutos. Si la sesión es muy larga, prioriza ejercicios multiarticulares y reduce el número de ejercicios de aislamiento.`;
  }
